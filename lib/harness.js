 ...
 */
export function buildLaunch({ harness, name, prompt }) {
  ...
  switch (harness) {
    case 'claude': {
      ...
      const argv = ['claude', '--name', name, '--mcp-config', cfg, '--allowedTools', 'mcp__agora', '--append-system-prompt', system];
    ...
    default:
      throw new Error(`unknown harness "${harness}" (expected claude, codex or gemini)`);
  }
}
