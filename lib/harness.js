...
// The agent's identity reaches the MCP server through the QORAL_AGENT env var, which agy passes through.
...
export function ensureAgyIntegration() {
  ...
  const have = mcp.mcpServers.qoral;
  if (!have || have.command !== want.command || JSON.stringify(have.args) !== JSON.stringify(want.args) || have.disabled) {
    mcp.mcpServers.qoral = { ...(have || {}), ...want };
    fs.mkdirSync(path.dirname(AGY_MCP_CONFIG), { recursive: true });
    fs.writeFileSync(AGY_MCP_CONFIG, JSON.stringify(mcp, null, 2) + '\n');
    notices.push(`registered the qoral MCP server for Antigravity in ${AGY_MCP_CONFIG} (remove with: agy mcp remove qoral)`);
  }
  // 2. Pre-approve qoral tools so the bus never blocks on "Do you want to proceed?"
  const settings = readJson(AGY_SETTINGS, {});
  settings.permissions ??= {};
  settings.permissions.allow ??= [];
  const rules = TOOL_NAMES.map((t) => `mcp(qoral/${t})`);
  ...
  if (missing.length) {
    ...
    notices.push(`added ${missing.length} mcp(qoral/*) allow rules to ${AGY_SETTINGS} so agents aren't prompted for bus calls`);
  }
  return notices;
}
...
export function buildLaunch({ harness, name, prompt, cwd }) {
  ...
  const mcpJson = { mcpServers: { qoral: { command: NODE, args: mcpArgs } } };
  ...
  switch (harness) {
    case 'claude': {
      ...
      // mcp__qoral pre-approves every tool from the qoral server so the bus never blocks on a permission prompt.
      ...
      const argv = [
        ...
        '--allowedTools', 'mcp__qoral', '--append-system-prompt', system,
      ];
    ...
    case 'codex': {
      ...
      const argv = [
        'codex',
        '-c',
        `mcp_servers.qoral.command=${JSON.stringify(NODE)}`,
        '-c',
        `mcp_servers.qoral.args=${JSON.stringify(mcpArgs)}`,
        fullPrompt,
      ];
      return { argv, env: {} };
    }
    ...
    case 'gemini': {
      ...
      // trust:true skips Gemini's per-tool confirmation for the qoral server.
      const geminiJson = { mcpServers: { qoral: { ...mcpJson.mcpServers.qoral, trust: true } } };
