...
export async function runAgent(name) {
  ...
  process.stdout.write(`\n${Y}[agora] documenting session…${R} ${D}(writes .agora/notes and .agora/KNOWLEDGE.md in ${spec.cwd})${R}\n`);
  try {
    const log = (m) => process.stdout.write(`${D}[agora] ${m}${R}\n`);
    ...
    if (res.skipped) process.stdout.write(`${D}[agora] not documented: ${res.reason}${R}\n`);
    else process.stdout.write(`${D}[agora] note: ${res.notePath}${res.knowledgePath ? `\n[agora] knowledge: ${res.knowledgePath}` : ''}${R}\n`);
  } catch (e) {
    process.stdout.write(`${Y}[agora] documentation failed:${R} ${e.message}\n`);
  }

  db.markExited(name);
  process.stdout.write(`\n${Y}[agora] agent "${name}" exited${R} (${code}). Press Enter to close this window.\n`);
