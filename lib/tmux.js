...
export const UI = 'qoral'; // outer session: sidebar | nested attach to `agents`
...
export function ensureAgentsSession() {
  ...
  // server / global options (optional ones via tmuxTry so an older tmux, e.g. macOS + Homebrew 3.2, still works)
  tmux('set', '-s', 'escape-time', '10');
  tmuxTry('set', '-s', 'focus-events', 'on');
  tmux('set', '-g', 'mouse', 'on');
  tmux('set', '-g', 'history-limit', '50000');
  // macOS ships an ncurses without the tmux-256color entry; fall back so apps inside don't break.
  const term = spawnSync('infocmp', ['tmux-256color'], { stdio: 'ignore' }).status === 0 ? 'tmux-256color' : 'screen-256color';
  tmux('set', '-g', 'default-terminal', term);
  tmuxTry('set', '-ga', 'terminal-overrides', ',*:Tc');
  tmuxTry('set', '-g', 'set-clipboard', 'on');
  tmuxTry('set', '-g', 'allow-passthrough', 'on');
  ...
  // Keep everything agents print in scrollback so the whole session can be documented on exit.
  tmux('set', '-t', AGENTS, 'alternate-screen', 'off');
...
export function ensureUISession() {
  ...
  const t = loadTheme().tmux;
  tmux('set', '-t', UI, 'status', 'on');
  tmux('set', '-t', UI, 'status-position', 'bottom');
  tmux('set', '-t', UI, 'status-style', `bg=${t.bg},fg=${t.fg}`);
  ...
  const k = (key, label) => `#[fg=${t.key}]${key}#[fg=${t.fg}] ${label}  `;
  tmux(
    'set',
    '-t',
    UI,
    'status-left',
    ` #[bold,fg=${t.accent}]qoral#[default,fg=${t.fg}]  ` +
      k('M-n', 'new') + k('M-m', 'message') + k('M-←/→', 'focus sidebar/agent') +
      k('M-[ M-]', 'prev/next agent') + k('M-1..9', 'jump') + k('M-d', 'detach'),
  );
  tmux('set', '-t', `${UI}:main`, 'pane-border-style', `fg=${t.border}`);
  tmux('set', '-t', `${UI}:main`, 'pane-active-border-style', `fg=${t.accent}`);
...
/** Entire scrollback + screen of a pane/window, wrapped lines joined. */
export function captureAll(target) {
  return tmuxTry('capture-pane', '-p', '-J', '-t', target, '-S', '-', '-E', '-') ?? '';
}
...
/** Tee everything the pane outputs into a file (raw, with escape sequences). */
export function pipePane(target, file) {
  return tmuxTry('pipe-pane', '-t', target, '-o', `cat >> ${q(file)}`) !== null;
}
