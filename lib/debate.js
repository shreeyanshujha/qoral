// `agora debate`: several agents argue a question through the bus under a deterministic
// moderator (this process), converge or time out, and a headless model writes the decision
// into <project>/.agora/decisions/ and links it from KNOWLEDGE.md. Optionally a builder implements it.
...
function debaterBrief({ name, persona, question, cwd, participants, rounds }) {
  ...
  return [
    `You are "${name}", one of ${participants.length} participants (${participants.join(', ')}) in a structured debate run by "${MODERATOR}". The moderator is an automated process on the agora bus, not a person: it only reads messages sent to it and relays positions between participants.`,
    ``,
    `QUESTION: ${question}`,
    ``,
    ...
    `1. Investigate briefly, then send your opening position with agora_send(to="${MODERATOR}"): at most 250 words, concrete, citing files where relevant. End the message with exactly these two lines:`,
    `   STANCE: <one-line summary of the approach you advocate>`,
    `   CONSENSUS: no`,
    `2. Immediately call agora_wait(timeout_seconds=600). The moderator will send you the other participants' positions.`,
    ...
    `4. After each reply call agora_wait(timeout_seconds=600) again. Stop when the moderator says the debate is over.`,
    ``,
...
function roundMessage({ round, rounds, positions, me }) {
  ...
  parts.push(
    ``,
    `Respond now: critique these, update your position if their argument is better, and reply to ${MODERATOR} ending with your STANCE and CONSENSUS lines. Then agora_wait again.`,
  );
  return parts.join('\n');
}
      ...
      while (Object.keys(got).length < names.length && Date.now() < deadline) {
        ...
        for (const n of names) {
          ...
          if (a?.status === 'attention') {
            warned.add(`${round}:${n}`);
            log(`[${elapsed()}] ${n} is waiting on a prompt in its window (permission/trust). Answer it in agora to let it continue.`);
    ...
    if (build) {
      ...
      const a = spawnAgent({
        ...
        prompt:
          ...
          `Follow the project's conventions, add or update tests as the plan says, run the test suite, and when everything passes send the human a short report with agora_send(to="human") listing the files you changed. ` +
