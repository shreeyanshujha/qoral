...
export async function runDoctor() {
  ...
  const { loadTheme, listThemes } = await import('./theme.js');
  const active = loadTheme().name;
  lines.push(truecolor ? ok(`theme: ${active} (truecolor available; hex themes render exactly)`) : warn(`theme: ${active} (no truecolor: hex themes approximate to 256 colours; still fine)`));
  lines.push(`${D}  available: ${listThemes().join(', ')} · preview with \`qoral theme\`${X}`);
