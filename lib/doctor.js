// `agora doctor`: check the machine for everything agora needs and print platform-specific advice.
...
export function runDoctor() {
  ...
  lines.push(`${B}agora ${VERSION}${X} · ${p} · ${os.release()} · ${BIN}`);
  lines.push('');
  ...
  if (process.platform === 'win32') {
    lines.push(bad('tmux: not available on native Windows. Run agora inside WSL2 (see below).'));
    problems++;
  } else {
    const tv = run('tmux', ['-V']);
    if (!tv) {
    ...
    lines.push(ti ? ok('terminfo: tmux-256color') : warn(`terminfo lacks tmux-256color; agora will use screen-256color inside tmux${process.platform === 'darwin' ? ' (fix: brew install ncurses, or add the entry with tic)' : ''}`));
  }
  ...
  if (process.platform === 'darwin') {
    ...
    lines.push(`${D}- agora's multiplexer is tmux, which has no native Windows build. Use WSL2:${X}`);
    lines.push(`${D}    wsl --install            (once, then reboot)${X}`);
    lines.push(`${D}    then inside WSL: install node ≥ 22.13 and tmux, clone agora, run ./install.sh${X}`);
    lines.push(`${D}- windows\\install.ps1 puts an "agora" command on your PowerShell PATH that runs it inside WSL.${X}`);
  } else if (isWSL()) {
    lines.push(`${D}- Windows Terminal binds Alt+arrows to pane focus; use Alt+h / Alt+l in agora, or unbind them in Windows Terminal settings.${X}`);
    lines.push(`${D}- Keep your projects on the Linux filesystem (~/…), not /mnt/c, for speed.${X}`);
  } else {
    lines.push(`${D}- Any terminal works (Foot, Alacritty, Kitty, GNOME Terminal, Konsole …); agora runs inside tmux.${X}`);
    lines.push(`${D}- If Alt chords are eaten by your window manager, use the sidebar keys or rebind in lib/tmux.js.${X}`);
  }

  lines.push('');
  lines.push(problems ? bad(`${problems} problem(s) to fix before agora will run`) : ok('ready: run `agora`'));
