...
import { HARNESSES, expandHome } from './paths.js';
...
const SPIN = ['◐', '◓', '◑', '◒'];
const HARNESS_TAG = { claude: `${S.orange}claude${S.reset}`, codex: `${S.green}codex${S.reset}`, gemini: `${S.blue}gemini${S.reset}` };
...
export async function runUI() {
  ...
  function renderMain(W, H) {
    ...
    if (state.flash && state.flash.until > Date.now()) {
      ...
      lines.push(` ${k('n', 'new')} ${k('m', 'msg')} ${k('b', 'bcast')} ${k('x', 'kill')} ${k('⏎', 'show')}`);
      lines.push(` ${k('l', 'log')} ${k('?', 'help')} ${k('d', 'detach')} ${k('Q', 'quit all')}`);
    }
    return lines;
  }
  ...
  function renderHelp(W) {
    ...
    return [
      ...
      k('n', 'new agent (harness, name, dir, task)'),
  ...
  async function newAgentWizard() {
    ...
    try {
      ...
      flash(`started ${a.name} (${a.harness})`);
    } catch (e) {
