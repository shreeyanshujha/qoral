...
// S carries structural styles plus the theme's semantic colors. Legacy role aliases
// (orange/green/…) map onto theme colors so the rest of the file reads unchanged.
const S = {
  ...theme.S,
  gray: theme.S.softdim,
  orange: theme.S.accent,
  green: theme.S.idle,
  yellow: theme.S.working,
  red: theme.S.attention,
  cyan: theme.S.documenting,
  magenta: theme.S.mail,
  blue: theme.S.harness.gemini,
  white: theme.S.text,
};
const ANSI_RE = /\x1b\[[0-9;]*m/g;
const vlen = (s) => s.replace(ANSI_RE, '').length;
const SPIN = Array.isArray(theme.glyphs.working) ? theme.glyphs.working : [theme.glyphs.working];
const GL = theme.glyphs;
const HARNESS_LABEL = { claude: 'claude', codex: 'codex', agy: 'agy', gemini: 'gemini', moderator: 'debate' };
const HARNESS_TAG = Object.fromEntries(
  Object.entries(HARNESS_LABEL).map(([h, label]) => [h, `${theme.S.harness[h] || ''}${label}${S.reset}`]),
);
...
export async function runUI() {
  ...
  function statusIcon(a) {
    switch (a.status) {
      case 'idle': return `${S.idle}${GL.idle}${S.reset}`;
      case 'working': return `${S.working}${SPIN[state.frame % SPIN.length]}${S.reset}`;
      case 'attention': return `${S.attention}${S.bold}${GL.attention}${S.reset}`;
      case 'documenting': return `${S.documenting}${GL.documenting}${S.reset}`;
      case 'moderating': return `${S.moderating}${GL.moderating}${S.reset}`;
      case 'exited': return `${S.exited}${GL.exited}${S.reset}`;
      default: return `${S.starting}${GL.starting}${S.reset}`;
    }
  }
  ...
  function renderMain(W, H) {
    ...
    const title = ` ${S.bold}${S.orange}qoral${S.reset}`;
    ...
    if (!busLines.length) busLines.push(`${S.dim} quiet. agents talk via qoral_send${S.reset}`);
  ...
  function renderLog(W, H) {
    ...
    const lines = [` ${S.bold}${S.orange}qoral${S.reset} ${S.dim}message log${S.reset}`, `${S.gray}${'─'.repeat(W)}${S.reset}`];
  ...
  function renderHelp(W) {
    ...
    return [
      ` ${S.bold}${S.orange}qoral${S.reset} ${S.dim}help${S.reset}`,
      ...
      k('l', 'message log'),
      k('o', "open project's .qoral/KNOWLEDGE.md"),
      ...
      `${S.dim} qoral_send · qoral_inbox · qoral_wait${S.reset}`,
      `${S.dim} qoral_spawn · qoral_list_agents · qoral_log${S.reset}`,
  ...
  async function debateWizard() {
    ...
    const cmd =
      ...
      `echo; echo "[qoral] debate finished. Press Enter to close."; read _`;
    try {
  ...
  function openKnowledge() {
    ...
    const file = `${cwd}/.qoral/KNOWLEDGE.md`;
    const cmd = `sh -c 'if [ -f "$1" ]; then \${EDITOR:-less} "$1"; else echo "no knowledge yet at $1"; echo; echo "(sessions are documented when they end; agents can also call qoral_remember)"; read _; fi' sh ${JSON.stringify(file)}`;
    try {
  ...
  async function quitAll() {
    const yes = await askConfirm('quit qoral and kill every agent? (y/N)');
    backToList();
    if (yes) {
