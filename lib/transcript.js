...
//   2. the raw pane output log qoral taps with `tmux pipe-pane` from launch
...
// ---- Antigravity CLI (agy) -------------------------------------------------
// Conversations are SQLite files of protobuf blobs under ~/.gemini/antigravity-cli/conversations/.
// We do not decode the schema; we pull readable UTF-8 runs out of each step's payload, which
// yields the user/assistant text and tool JSON in order. Best effort, but far richer than a screen redraw log.

const UUID_RE = /[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/gi;

function printableRuns(buf, min = 24) {
  const s = buf.toString('utf8');
  const runs = s.match(/[\x20-\x7e -￿\n\t]{24,}/g) || [];
  // drop bare ids and protobuf framing; keep runs that contain at least four real words
  return runs
    .map((r) => r.replace(UUID_RE, '').replace(/^[^A-Za-z{["/]+/, '').trim())
    .filter((r) => r.length >= min && (r.match(/[A-Za-z]{2,}/g) || []).length >= 4);
}

export function agyTranscript({ cwd, startedAt }) {
  const dir = path.join(os.homedir(), '.gemini', 'antigravity-cli', 'conversations');
  const dbs = globDir(dir, (f) => f.endsWith('.db')).filter((f) => {
    try {
      return fs.statSync(f).mtimeMs >= (startedAt ?? 0) - 60_000;
    } catch {
      return false;
  ...
  for (const f of dbs) {
    try {
      ...
      const meta = db.prepare(`SELECT data FROM trajectory_metadata_blob LIMIT 1`).get();
      const metaText = meta?.data ? Buffer.from(meta.data).toString('utf8') : '';
      const matchesCwd = !cwd || metaText.includes(cwd);
      const rows = db.prepare(`SELECT idx, step_payload FROM steps ORDER BY idx ASC`).all();
      db.close();
      const lines = [];
      const seen = new Set();
      for (const r of rows) {
        if (!r.step_payload) continue;
        for (const run of printableRuns(Buffer.from(r.step_payload))) {
          if (seen.has(run)) continue;
          seen.add(run);
          lines.push(run);
        }
      }
      candidates.push({ f, matchesCwd, mtime: fs.statSync(f).mtimeMs, text: lines.join('\n') });
    } catch {}
  }
  candidates.sort((a, b) => Number(b.matchesCwd) - Number(a.matchesCwd) || b.mtime - a.mtime);
  const best = candidates[0];
  if (!best || !best.text.trim()) return null;
  return { source: `agy conversation ${path.basename(best.f)}`, text: best.text };
}
...
export function collectTranscript({ harness, cwd, sessionId, startedAt, rawLog, paneTarget }, minChars = 1500) {
  ...
  if (harness === 'agy') tryAdd(() => agyTranscript({ cwd, startedAt }));
