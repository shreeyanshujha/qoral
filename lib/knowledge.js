...
export const KNOWLEDGE_DIRNAME = '.qoral';
...
export function loadConfig() {
  const defaults = { summarizer: 'auto', model: null, document_sessions: true, max_agents: 8, theme: 'default' };
  try {
...
// agy last: its headless mode may try to run tools and get auto-denied, producing no output.
const SUMMARIZER_ORDER = ['claude', 'codex', 'gemini', 'agy'];

export function pickSummarizer(cfg = loadConfig()) {
  const want = process.env.QORAL_SUMMARIZER || cfg.summarizer || 'auto';
  if (want === 'none') return null;
  if (want !== 'auto') return HARNESSES.includes(want) && which(want) ? want : null;
  return SUMMARIZER_ORDER.find(which) ?? null;
}
...
export function knowledgeSection(cwd) {
  ...
  const lines = [];
  lines.push(`## Project knowledge (accumulated by earlier qoral sessions in ${cwd})`);
  if (know) {
    lines.push(know.length > KNOWLEDGE_CAP ? know.slice(0, KNOWLEDGE_CAP) + '\n…(truncated; read .qoral/KNOWLEDGE.md for the rest)' : know);
  } else {
  ...
  lines.push(
    '',
    'Record durable facts for future agents with qoral_remember(text): architecture, conventions, decisions, gotchas, how to run/test. ' +
      'When your session ends it is summarized automatically into .qoral/notes and merged into .qoral/KNOWLEDGE.md.',
  );
  return lines.join('\n');
}
...
function buildPrompt({ name, harness, cwd, task, startedAt, endedAt }) {
  ...
  return `You are writing documentation for a coding-agent session that just ended inside the "qoral" multi-agent workspace.
...
export function runSummarizer(prompt, input, cwd, { summarizer, cfg } = {}) {
  ...
  if (!summarizer) throw new Error('no summarizer available (set summarizer in ~/.local/share/qoral/config.json)');
  const model = process.env.QORAL_SUMMARIZER_MODEL || cfg.model;
  ...
  switch (summarizer) {
    ...
    case 'gemini': // stdin is appended to -p prompt
      ...
      stdin = input;
      break;
    case 'agy': // print mode; everything goes in the prompt argument
      argv = ['agy', '-p', `${prompt}\n\n${input}`, '--disable-slash-commands', ...withModel('--model')];
      stdin = '';
      break;
    default:
  ...
  const r = spawnSync(argv[0], argv.slice(1), {
    ...
    encoding: 'utf8',
    cwd,
    env: { ...process.env, QORAL_AGENT: undefined },
...
/**
 * Summarize a session transcript into .qoral/notes/<stamp>-<agent>.md and merge into .qoral/KNOWLEDGE.md.
