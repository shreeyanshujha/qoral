...
export function formatNudge(agent, msgs) {
  ...
  if (msgs.length === 1) {
    const m = msgs[0];
    text = `[agora] message from ${m.sender}: ${flatten(m.body)}`;
  } else {
    text =
      `[agora] ${msgs.length} new messages: ` +
  ...
  const replyHint =
    senders.length === 1
      ? `Reply with agora_send(to="${senders[0]}").`
      : `Reply to each sender with agora_send.`;
  text += truncated ? ` [truncated; call agora_inbox for the full text.] ${replyHint}` : ` ${replyHint}`;
  return text;
}
