...
export async function runHome() {
  ...
  const lines = [
    '',
    `  ${B}${Y}qoral${R}  ${D}many agents, one room${R}`,
    '',
    `  Nothing selected yet. Start an agent from the sidebar on the left:`,
    '',
    `    ${B}n${R}        new agent (pick claude / codex / agy / gemini, a name, a directory, a task)`,
    ...
    `    ${B}Alt+d${R}    detach (agents keep running; run ${B}qoral${R} to come back)`,
    '',
    `  Agents can talk to each other through the ${B}qoral${R} MCP tools`,
    `  (qoral_send, qoral_wait, qoral_spawn, ...). Messages to an idle agent are`,
    `  typed straight into its prompt, so conversations flow without polling.`,
    '',
    `  ${D}CLI: qoral spawn claude --dir ~/proj "task"   qoral send ada "hi"   qoral list   qoral log${R}`,
    '',
  ];
